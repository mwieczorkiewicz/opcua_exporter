...
func TestReadNodeFile(t *testing.T) {
	...
	results, err := parse(bytes.NewReader(data))
	assert.NoError(t, err)
	assert.Equal(t, len(testNodes), len(results))
	assert.IsType(t, NodeMapping{}, results[0])
	...
	results, err = parse(strings.NewReader("foooob not valid json here"))
	assert.Error(t, err)
...
func TestB64Config(t *testing.T) {
	...
	results, err := ReadBase64(&encodedData)
	assert.NoError(t, err)
	assert.Equal(t, len(testNodes), len(results))
	assert.IsType(t, NodeMapping{}, results[0])
	assert.Equal(t, testNodes[0].NodeName, results[0].NodeName)
...
func TestJsonConfig(t *testing.T) {
	...
	results, err := parse(strings.NewReader((json)))
	assert.NoError(t, err)
